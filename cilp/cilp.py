...
def val_step(data_loader, model, criterion):
    ...
    for X, y in data_loader:
        ...
        y_pred.append(y_logit)
        y_pred_acc.append((to_numpy(y_logit) >= 0).astype(int))
        y_true.append(to_numpy(y).astype(int))

    y_pred = np.concatenate(y_pred)
    y_pred_acc = np.concatenate(y_pred_acc)
...
class MLP(nn.Module):
    def __init__(self, params):
        ...
        #layers.append(nn.Softmax(output_feats))
        #print(layers)
    ...
    def predict(self, x):
        ...
        return y_logit #(to_numpy(y_logit) >= 0).astype(int)
...
class CILP:
    ...
    def semi_prop(self):
        run_semi_prop(self.data_dir, self.h_arity, cached=self.cached, print_output=False)

    def featurise(self):
        if self.semi_prop:
            examples_dict = load_json(pjoin(self.data_dir, 'bc_filtered.json'))
        else: 
