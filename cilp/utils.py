...
def apply_laplacian_smooth(examples,n_positives,n_negatives):
    examples_df = pd.DataFrame(np.matrix(examples))
    examples_df['label'] = 'NaN'
    examples_df.iloc[:n_positives, examples_df.columns.get_loc('label')] = 1 
    examples_df.iloc[n_positives:, examples_df.columns.get_loc('label')] = 0
    alpha = 1e-5

    column_names = examples_df.columns.values.tolist()[:-1]

    for column_name in column_names:
        train_unique_feature_values = examples_df[examples_df['label'] == category][column_name].unique()
        
        data_unique_feature_values = examples_df[column_name].unique()
        
        for feature_value in data_unique_feature_values:
            if feature_value not in train_unique_feature_values:
                examples_df[column_name][feature_value] = alpha/(examples_df[examples_df['label'] == category].shape[0] + (data_unique_feature_values.shape[0]*alpha))
            else:
                examples_df[column_name][feature_value] = np.count_nonzero(examples_df[examples_df['label'] == category][column_name] == feature_value)/examples_df[examples_df['label'] == category].shape[0]
                
    return examples_df


def get_features(bcp_examples, n_positives, n_negatives):
